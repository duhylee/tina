...
#include "u_audio.h"
#include "../../../../sound/soc/codecs/ad82584f.h"
...
int u_audio_get_volume(struct g_audio *audio_dev, int playback, s16 *val)
{
	...
	*val = prm->volume;
	...
	if (playback) {
//		printk("=====> [%s] Playback - volume : %d\n",__func__, prm->volume);
	} else{
//		printk("=====> [%s] Capture - volume : %d\n",__func__, prm->volume);
		vol = ad82584f_get_volume_ext();
//		printk("=====> [%s] ad82584f volume : %d\n",__func__, vol);
	}
...
//			printk("=====> [%s] Capture - volume : %d\n",__func__, prm->volume);
			volume = (prm->volume - prm->volume_min) / prm->volume_res;
//			printk("=====> [%s] converted volume : %d\n",__func__, volume);
			ad82584f_set_volume_ext(volume);
		}
	}
	spin_unlock_irqrestore(&prm->lock, flags);
