...
function build_boot()
{
	...
	bin_dir="target/allwinner/${TARGET_PLATFORM}-common/bin"
	if [ "${TARGET_UBOOT}" = "u-boot-2018" ]; then
		...
		[ x"$o_option" = x"uboot" -a -f "u-boot-2018/configs/${TARGET_BOARD}_defconfig" ] && {
			...
			bin_dir="target/allwinner/${TARGET_BOARD}/bin"
			mkdir -p $T/${bin_dir}
		}
	else
		\cd $T/lichee/brandy/
...
function swupdate_init_key() {
	...
	local SWUPDATE_CONFIG_DIR="$T/target/allwinner/${TARGET_BOARD}/configs"
...
function swupdate_pack_swu() {
	...
	local SWUPDATE_CONFIG_DIR="$T/device/config/chips/${TARGET_PLATFORM}/configs/${TARGET_PLAN}"
